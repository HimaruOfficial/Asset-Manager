          ...
          <>
            ...
            <View style={styles.summaryRow}>
              <View style={[styles.summaryCard, { backgroundColor: colors.income + "15", borderColor: colors.income + "30" }]}>
                ...
                <Text style={[styles.summaryAmount, { color: colors.income }]}>${totals.income.toLocaleString()}</Text>
              ...
              <View style={[styles.summaryCard, { backgroundColor: colors.expense + "15", borderColor: colors.expense + "30" }]}>
                ...
                <Text style={[styles.summaryAmount, { color: colors.expense }]}>${totals.expense.toLocaleString()}</Text>
            ...
            <View style={[styles.filterRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
              {(["all", "income", "expense"] as Filter[]).map((f) => (
                <TouchableOpacity
                  key={f}
                  style={[styles.filterTab, filter === f && { backgroundColor: f === "income" ? colors.income : f === "expense" ? colors.expense : colors.primary }]}
                  onPress={() => setFilter(f)}
                >
            ...
            {filtered.length === 0 && (
              <View style={[styles.empty, { borderColor: colors.border }]}>
                ...
                <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>Tap + to log your first one</Text>
              </View>
            )}
          </>
        }
